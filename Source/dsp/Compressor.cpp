...
void Compressor::process(AudioBuffer<float>& buffer, int num_samples)
{
    if (!bypassed)
    {
        ...
        FloatVectorOperations::max(rawSidechainSignal, rawSidechainSignal, buffer.getReadPointer(0), numSamples);
